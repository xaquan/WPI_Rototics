...
class KinematicModel():
    ...
    def inverse_kinematics_ABB_1400(self, position, orientation):
        """
            ...
            a desired end-effector position and orientation. It uses analytical methods to solve
            the inverse kinematics problem for a robot with the specified DH parameters.
            Args:
                position (array-like): The desired end-effector position as a 3D vector [px, py, pz]
                                    in the base frame coordinates (in mm).
                orientation (array-like): A quaternion [x, y, z, w] representing the desired 
                                        end-effector orientation relative to the base frame.
            Returns:
            ...
            Notes:
                - The function uses robot-specific DH parameters: d1=475, a1=150, a2=600, a4=120,
                ...
                - The function assumes the input orientation matrix is valid and a solution exists.
                - Angles are returned in the range [-π, π].
            """
        # Convert quaternion to rotation matrix
        # orientation = QuaternionConverter.quat_to_rotation_array(*orientation)
        rotation = ConverterHelper.quat_to_rotation_array(orientation[0], orientation[1], orientation[2], orientation[3]) 
        ...
        px = position[0] - d6 * r13
        py = position[1] - d6 * r23
        pz = position[2] - d6 * r33
        ...
        theta1_candidates = np.array([np.arctan2(C, D), np.arctan2(-C, D)])
        k = np.argmin(np.abs(self.wrap_to_pi(theta1_candidates)))
        theta[0] = theta1_candidates[k]

        # ---------------------------
        # Theta2
        G = (a2**2 + r6**2 - r5**2) / (2.0 * a2 * r6)  # cos(beta)
        # G = _clamp01(G)
        H = np.sqrt(1.0 - G**2)                         # sin(beta)
        ...
        theta3_candidates = np.array([np.pi/2.0 - gamma + sigma1, np.pi/2.0 - gamma + sigma2])
        ...
        # Solve theta4
        D4 = c1*c23*r13 + s1*c23*r23 + s23*r33   # c4s5 (per your comments)
        C4 = s1*r13 - c1*r23      # s4s5 (per your comments)
        theta4 = np.arctan2(D4, C4)              
        theta[3] = theta4            # NOTE: kept same atan2 order as your MATLAB

        # Solve theta5
        D5 = c1*s23*r13 + s1*s23*r23 - c23*r33  # c5 (per your comment)
        # D5v = _clamp01(D5v)
        C5 = np.sqrt(1.0 - D5**2)              # sqrt(1 - c5^2)
        ...
        # Solve theta6
        D6 = -(c1*s23*r11 + s1*s23*r21 - c23*r31)  # -s5c6 (per your comment)
        C6 =  (c1*s23*r12 + s1*s23*r22 - c23*r32)  #  s5s6 (per your comment)
